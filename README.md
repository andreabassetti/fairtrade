...
Our Machine learning model reads in a csv file that is edited from the Fair Trade USA Excel file we started with. We use the NPS scoring system (This is a widely used score, here is more info about it: https://www.netpromoter.com/know/).
 - Description of preliminary data preprocessing: Angela found the original database and explained each column and we did a preliminary selection of which columns we were confident we did not need. Then we removed rows that included ‘Refused’, ‘Don’t know’, and ‘null’. Then we converted data to the correct types we needed. We then dropped even more columns that were not necessary for determining job satisfaction in our model.  This was done to maintain as many rows of data as possible. We then did some further editing to drop data that was incomplete, so we had a clean data set. Using dictionaries, we then change response from: very good, somewhat good, neither good or bad, somewhat bad, and very bad; to number values 5-1. We did similar conversions to all similar responses. When we decided on using a binary system, we had to make the location column several columns to accommodate it. For the rest of the columns we used the NPS scoring system, but instead of ternary like last time we now used binary.
 - Description of preliminary feature engineering: The top of the 3 features are ‘age_core’ (17%), ‘entity_tenure_total_months’ (16%), and ‘conditions_rate_core’(12%). A combined total of 45%. We dropped variables that of top variables and why we picked variables and why we dropped variables
 - Description of how data was split into training and testing set: 30% of the data was split into the testing set and 70% into the training set
 - Explanation of the model choice: We tried 3 models: Random Forest Classifier; Adaboost classifier; and Oversampling. We choose the Random Forest Classifier since we wanted to avoid overfitting and we could use it to rank the importance of the input features.
